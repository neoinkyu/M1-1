...
이때 각 구간의 클릭수, 노출수, CTR, 평균 검색순위, 사용자, PV, AdSense 예상수익, 게시물 수는 **해당 28일 동안의 일평균 값**으로 계산하였다. 따라서 28일 구간별 비교는 각 구간의 총합이 아니라, 서로 길이가 동일한 28일 구간의 평균적인 일별 성과를 비교하기 위한 분석이다.
...
이 분석에서는 전체 112일 중 **초기 28일(2026-06-02 ~ 2026-06-29)**과 **최근 28일(2026-08-25 ~ 2026-09-21)**을 직접 비교하였다. 페이지별·검색어별 수치는 각 28일 구간의 **누적 노출수와 누적 클릭수 합계**이며, 가운데 56일은 해당 초기-최근 직접 비교값에는 포함하지 않았다. 전체 112일은 별도의 추세 및 구간 분석에 사용하였다.
...
Pearson 상관계수는 두 변수의 **선형 관계의 방향과 강도**를 기술하기 위해 사용하였다. 본 분석에서는 별도의 p-value 유의성 검정은 수행하지 않았다. 일반적인 Pearson 유의성 검정은 관측값 간 독립성을 전제로 하는데, 이번 자료는 연속된 일별 시계열이므로 특정 날짜의 값이 전날이나 다음 날의 값과 연관되는 **자기상관(autocorrelation)**이 존재할 수 있고, 7일 주기의 계절성도 확인되었다. 따라서 일반적인 Pearson p-value를 그대로 적용하면 실질적인 독립 정보량보다 표본 수를 크게 보는 문제가 생길 수 있다. 이에 본 분석에서는 상관계수를 기술적 지표로 해석하고 통계적 유의성을 별도로 주장하지 않았다.
...
가법적(additive) 분해는 개념적으로 다음과 같이 해석하였다.

`Observed = Trend + Seasonal + Residual`

각 요소의 의미는 다음과 같다.

- **Observed**: Google Search Console에서 수집한 실제 일별 검색 클릭수
- **Trend**: 단기 변동과 반복적인 요일 효과를 완화한 중기적인 흐름. `period=7`인 경우 기본적으로 중심 7일 이동평균에 기반해 추정된다.
- **Seasonal**: 7일 주기로 반복된다고 가정한 계절 성분
- **Residual**: Trend와 Seasonal로 설명되지 않는 불규칙한 나머지 변동

`period=7`은 Seasonal 그래프를 보고 사후적으로 정한 값이 아니라, 사전 탐색에서 요일별 클릭 차이와 주간 반복 패턴이 예상되었기 때문에 설정하였다. 따라서 Seasonal 그래프가 정확히 7일마다 같은 형태로 반복되는 것 자체는 `period=7` 설정에 따른 결과이며, 그것만으로 7일 계절성이 새롭게 발견되었다고 해석하지 않았다. 대신 실제 요일별 평균의 차이, 일별 시계열의 반복 패턴, 추정된 계절 성분의 크기를 함께 근거로 주간 계절성이 존재한다고 판단하였다.

가법적 방식을 선택한 것은 계절성을 추출하기 위한 필수 조건이기 때문이 아니라, 이번 분석에서는 요일 효과가 현재 클릭 수준에 비례해 배수로 커진다기보다 **일정한 절대 크기가 더해지거나 빠지는 형태로 단순화하는 것이 적절하다고 판단했기 때문**이다. 계절 변동의 크기가 데이터 수준에 비례해 커지는 구조라면 multiplicative 방식도 고려할 수 있다.
...
Holt-Winters Exponential Smoothing을 선택한 이유는 이번 검색 클릭 데이터에서 확인된 **추세와 7일 계절성을 동시에 반영할 수 있고**, 지수평활 방식의 특성상 일반적으로 **최근 관측값에 더 큰 가중치를 두고 과거 관측값의 영향은 점차 줄이는 구조**를 갖기 때문이다. 또한 복잡한 예측모형보다 구조와 가정을 설명하기 쉬워 본 과제의 간단한 베이스라인 예측에 적합하였다. 이번 모델은 정밀한 미래 예측보다는, 최근 수준·추세·주간 패턴이 이어진다는 가정 아래 기준선이 되는 예측을 만드는 목적이다.
...
Streamlit은 `pip install streamlit`로 설치하여 사용하는 **Python 패키지이자 데이터 앱 프레임워크**이다. 이번 프로젝트는 일반적인 웹서비스보다 Python 기반 데이터 분석 결과를 빠르게 탐색할 수 있는 대시보드를 만드는 것이 핵심이었기 때문에 Streamlit을 사용하였다.

주요 선택 이유는 다음과 같다.

- 기존 `pandas`, `matplotlib`, Google Sheets 연동 코드를 Python 안에서 그대로 재사용할 수 있음
- `st.date_input`, `st.metric`, `st.tabs`, `st.line_chart` 등으로 기간 선택·KPI·탭·그래프를 빠르게 구현할 수 있음
- 별도의 HTML/CSS/JavaScript 프론트엔드를 크게 작성하지 않아도 됨
- GitHub 저장소와 `requirements.txt`, Secrets 설정을 이용해 Streamlit Community Cloud에 비교적 간단하게 배포할 수 있음
- 과제의 보너스 요구사항인 '기간/조건을 바꿔보며 탐색하는 대시보드'와 목적이 잘 맞음

Vercel을 이용하는 것도 가능하지만, Vercel은 일반적으로 Next.js/React 기반의 웹 애플리케이션과 세밀한 프론트엔드 UI를 구성할 때 강점이 있다. 이번 과제에서는 별도의 프론트엔드와 API 계층을 구축하는 것보다 기존 Python 분석 코드를 직접 재사용하는 편이 단순하고 효율적이므로 Streamlit을 선택하였다.
...
- `pandas`: Google Sheets에서 불러온 데이터의 전처리, 병합, 집계 및 분석
- `numpy`: 수치 계산 지원
- `matplotlib`: 정적 분석 그래프 생성
- `statsmodels`: 시계열 분해 및 Holt-Winters Exponential Smoothing 예측
- `gspread`: Google Sheets API를 이용한 원본 데이터 조회
- `streamlit`: Python 분석 결과를 인터랙티브 웹 대시보드로 구현하고 실행
